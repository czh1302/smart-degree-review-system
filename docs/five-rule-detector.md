...
以“规则×PDF”为分类单位，TP=实际违规且检测器报违规，FN=实际违规却未报，FP=合规却报违规，TN=合规且未报。Recall=TP/(TP+FN)，Precision=TP/(TP+FP)，误报率=FP/(FP+TN)。现有 500 份 mutant 是已注入违规的阳性样本；按注入位置匹配，TP=500、FN=0，故仅能报告**变异阳性集的 Recall=100%**。母本及其他真实论文尚未逐条独立标注，不能计算 FP、TN、标准 Precision 或真实场景的全面 Recall。

数据集并非每份 mutant 恰好一处编辑：规则 6、18、22、24 各份一处，规则 28 各份在同一目录页编辑五个页码。因此“900 个位点”是 400+100×5 个已改文字位置，不是 900 份独立 mutant。若实验设计要求每份只改一处，必须重新生成并验收规则 28 的 100 份 PDF。
